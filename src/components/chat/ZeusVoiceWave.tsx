...
function randomLevels(): number[] {
  const center = Math.floor(2 + Math.random() * (SEGMENTS - 1));
...
export default function ZeusVoiceWave({ active }: { active: boolean }) {
  ...
  useEffect(() => {
    if (active) {
      ...
      intervalRef.current = setInterval(() => setLevels(randomLevels()), UPDATE_MS);
    } else {
