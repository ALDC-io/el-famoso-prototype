  ...
  if (message.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-br-sm border border-primary/30 bg-primary/15 px-3.5 py-2.5 text-sm text-white">
          {message.text}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-start">
      <div className="max-w-[85%] rounded-2xl rounded-bl-sm bg-white/5 px-3.5 py-2.5 font-mono text-sm text-cyan-100/90 shadow-sm shadow-cyan-500/5">
        {displayed}
        {!done && (
          <span className="animate-cursor-blink ml-0.5 inline-block h-4 w-[2px] translate-y-[2px] bg-primary" />
        )}
      </div>
    </div>
  );
